...
const mappedAbstractAriaRolesToDescriptions = {
  'composite': 'Composites are made up of multiple interactive widgets. Focus management may be handled by the composite element.',
  'landmark': 'Landmarks define top-level sections of a page that should appear in a site map or table of contents.',
  'structure': 'Structures represent non-interactive document semantics.',
  'widget': 'Widgets represent focusable, interactive elements. For user agents with browsing modes, widgets switch the mode to keyboard passthrough.',
  'window': 'Windows represent dialogs or windows that are children of the main window.',
...
const mappedAriaRolesToDescriptions = {
  ...
  'application': 'A region that represents an application. For user agents that support browse modes, this role switches to keyboard passthrough mode.',
  ...
  'columnheader': 'A cell containing header information for a column. This may be interactive if it is a descendant of a grid.',
  'rowheader': 'A cell containing header information for a row. This may be interactive if it is a descendant of a grid.',
  ...
  'document': 'A region that represents document content (as opposed to a web application.) For user agents that support browse modes, this role switches to document browse mode.',
  'emphasis': 'A range of text with a different semantic meaning than the surrounding content.',
  ...
  'figure': 'A figure is a block of content that is self-contained and typically referenced as a single unit from the main flow of the document.',
  ...
  'gridcell': 'A cell in a grid or treegrid. This is an interactive widget only if it is a descendant of a treetgrid.',
  ...
  'region': 'A large perceivable section of a web page or document, that typically contains a collection of items and objects.',
  ...
  'subscript': 'A subscript is a character that is set slightly below the normal line of type.',
  'superscript': 'A superscript is a character that is set slightly above the normal line of type.',
    ...
    <div>
      ...
      <style dangerouslySetInnerHTML={{__html: `
          ...
          */
          ...
          ${Object.entries(mappedAbstractAriaRolesToBackgroundColors).map(([role, color]) => {
            return `
      ...
      <ul className="periodic-table__root">
        {Object.entries(abstractAriaRolesByType).map(([type, abstractAriaRoles]) => {
          return (
            <li className="periodic-table__subgrid periodic-table__subgrid-area" key={type}>
              <h2 className="periodic-table__subgrid-area-heading">
                {type}
              </h2>
              <ul className="periodic-table__subgrid periodic-table__subgrid-row" style={{
              }}>
                {abstractAriaRoles.map((abstractAriaRole) => {
                  const ariaRoles = (ariaRolesByCategory[abstractAriaRole] || []).sort(
                    (a?: string, b?: string) => a?.localeCompare(b || '')
                  );
                  const title = mappedAbstractAriaRolesToTitles[abstractAriaRole];
                  const description = mappedAbstractAriaRolesToDescriptions[abstractAriaRole];

                  return (
                    <li key={abstractAriaRole} className="periodic-table__subgrid periodic-table__subgrid-row" style={{
                    }}>
                      ...
                      <ul className="periodic-table__subgrid periodic-table__subgrid-row" style={{
                        ...
                        {ariaRoles.sort((a, b) => {
                          const isAPhrasing = (mappedAriaRolesToContentType[a] || []).includes('phrasing');
                          const isBPhrasing = (mappedAriaRolesToContentType[b] || []).includes('phrasing');
                          ...
                          const displayName = mappedAriaRolesToDisplayNames[role] || role;
                          ...
                          return (
                            <li
                              ...
                              className={`
                                ...
                                ${(mappedAriaRolesToContentType[role] || []).map((contentType) => `aria-role--content-type-${contentType}`).join(' ')}
                              `}
                              key={role}>
                              <expansion-button
                                role="button"
                                ...
                                <div className="aria-role__details">
                                  ...
                                  <table className="aria-role__table">
                                    <tr className="aria-role__row">
                                      <th className="aria-role__column-header" scope="col">
